...
|--------|--------|---------|
| `logging` | **Logging** | Structured logging with personal defaults |
| `config` | **Configuration** | Environment-based config injection |
| `security` | **Security** | FastAPI auth (JWT, API keys) |

## Usage

```python
from pykit import Logger, Config
from pykit.security.fastapi import Authenticator
```

## Structure Note

Currently flat. Promote to subpackages when any domain exceeds one module:
